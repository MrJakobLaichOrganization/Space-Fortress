...
Tilemap::Tilemap(const sf::Texture &tx,
	...
	: m_tx{tx}, m_tileDims{tileDims}, m_tileCount{tileCount}
{
...
void Tilemap::setTile(std::uint32_t idx, std::uint32_t tileIdx) {
	...
	vert[0].position = sf::Vector2f(pos.x * m_tileDims.x, pos.y * m_tileDims.y);
	vert[1].position =
		sf::Vector2f((pos.x + 1) * m_tileDims.x, pos.y * m_tileDims.y);
	vert[2].position =
		sf::Vector2f((pos.x + 1) * m_tileDims.x, (pos.y + 1) * m_tileDims.y);
	vert[3].position =
		sf::Vector2f(pos.x * m_tileDims.x, (pos.y + 1) * m_tileDims.y);
	...
	vert[2].texCoords =
		sf::Vector2f((tu + 1) * m_tileDims.x, (tv + 1) * m_tileDims.y);
